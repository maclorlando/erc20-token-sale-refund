...
import "module-1/contracts/MyToken.sol";
import "module-1/contracts/GodMode.sol";
...
contract TokenSale {
    ...
    function buyTokens() external payable {
        ...
        if (balance < TOKEN_BUY_AMOUNT) {
            ...
            require(mintRequired + myToken.totalSupply() <= myToken.cap(), "Unable to mint without exceeding MAX_SUPPLY");
            myToken.mint(address(this), mintRequired);
            balance = myToken.balanceOf(address(this));
            if (balance > 0) {
    ...
    function sellBack(uint256 amount) external payable returns(address){
        require(myToken.balanceOf(msg.sender) >= amount, "User does not hold this many tokens to sell back.");
